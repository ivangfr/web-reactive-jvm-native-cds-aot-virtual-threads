...
- Run the command below to start the application in:
  - JVM Mode
    ```bash
    docker run --rm --name quarkus-greetings-api-reactive \
      -p 8080:8080 \
      ivanfranchin/quarkus-greetings-api-reactive:3.31.1-21-jvm
    ```
  - Native Mode
    ```bash
    docker run --rm --name quarkus-greetings-api-reactive \
      -p 8080:8080 \
      ivanfranchin/quarkus-greetings-api-reactive:3.31.1-21-native
    ```
...
- To stop the application, press `Ctrl+C` in the terminal where it is running or run `docker stop quarkus-greetings-api-reactive`.
